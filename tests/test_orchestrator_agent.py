...
async def test_director_returns_actions():
    ...
    with (
        patch("src.agents.director_agent.ChatOpenAI.ainvoke", new_callable=AsyncMock) as mock_ainvoke,
        patch("src.agents.director_agent.cl.user_session.get", new_callable=MagicMock) as mock_user_session_get,
    ):
...
async def test_director_handles_exception():
    ...
    with (
        patch("src.agents.director_agent.ChatOpenAI.ainvoke", new_callable=AsyncMock, side_effect=Exception("fail")),
        patch("src.agents.director_agent.cl.user_session.get", new_callable=MagicMock) as mock_user_session_get,
    ):
        mock_user_session_get.return_value = {}
        actions = await _direct_actions(state)
