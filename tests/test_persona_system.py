...
async def test_workflow_filters_avoided_tools(monkeypatch, mock_cl_environment):
    ...
    # Patch the function referenced in agents_map['write'] (which is call_writer_agent from writer_agent)
    monkeypatch.setattr("src.agents.writer_agent.call_writer_agent", fake_writer)
