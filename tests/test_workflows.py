...
async def test_chat_workflow(mock_chat_state):
    ...
    with (
        ...
        # Add mocks for context-related dependencies
        patch("langgraph.config.get_config") as mock_get_config,
        patch("langchain_core.runnables.config.RunnableConfig") as mock_runnable_config,
    ):
        mock_get_config.return_value = {}  # Fake config
        mock_cl_session.get.return_value = MagicMock(vector_store=MagicMock())  # Ensure vector store exists
