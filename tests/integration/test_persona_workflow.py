...
async def test_persona_workflow_filters_and_reorders(monkeypatch):
    ...
    async def fake_therapist_workflow(inputs, state, **kwargs):
        return [
            AIMessage(
                content="Therapy response", name="therapist", metadata={"message_id": "w1"}
            )
        ]
    ...
    monkeypatch.setattr(
        owf, "persona_classifier_agent", AsyncMock(return_value={"persona": "Therapist"})
    )  # Assume classifier runs
