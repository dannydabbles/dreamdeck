...
async def test_command_roll(mock_session_data):
    ...
    ai_response_msg = AIMessage(
        content="Rolled 2d6: 7",
        name="dice_roll",
        metadata={"message_id": "ai-roll-msg-id"},
    )

    with patch("src.commands.cl.user_session.get", side_effect=user_session_get), patch(
        "src.commands.cl.user_session.set", new_callable=MagicMock
    ) as mock_user_session_set, patch(
        "chainlit.Message", new_callable=MagicMock
    ) as mock_cl_message_cls, patch(
        "src.commands.dice_agent",
        ...
        vector_store.put.assert_any_await(
            ...
            message_id="user-roll-msg-id",
            metadata={"type": "human", "author": "Player", "persona": "Default"},
        )
        vector_store.put.assert_any_await(
            content="Rolled 2d6: 7",
            message_id="ai-roll-msg-id",
            metadata={"type": "ai", "author": "dice_roll", "persona": "Default"},
        )
...
async def test_command_search(mock_session_data):
    ...
    ai_response_msg = AIMessage(
        ...
        name="web_search",
        metadata={"message_id": "ai-search-msg-id"},
    )
    ...
    with patch("src.commands.cl.user_session.get", side_effect=user_session_get), patch(
        ...
        # The state should now have 2 messages: user, tool
        assert len(state.messages) == 2
        assert isinstance(state.messages[0], HumanMessage)
        assert state.messages[0].content == f"/search {query}"
        assert state.messages[1] == ai_response_msg
        # Expect only 2 puts: user message and tool message
        assert vector_store.put.await_count == 2
        vector_store.put.assert_any_await(
        ...
        vector_store.put.assert_any_await(
            content="Search results...",
            message_id="ai-search-msg-id",
            metadata={"type": "ai", "author": "web_search", "persona": "Default"},
        )
...
async def test_command_todo(mock_session_data):
    ...
    ai_response_msg = AIMessage(
        ...
        name="todo",
        metadata={"message_id": "ai-todo-msg-id"},
    )
    ...
    with patch("src.commands.cl.user_session.get", side_effect=user_session_get), patch(
        ...
        "src.commands.call_todo_agent",
        new_callable=AsyncMock,
        return_value=[ai_response_msg],
    ) as mock_call_todo_agent:
        ...
        assert mock_cl_message_instance.author == "Player"
        # Only the user message is sent here; the AI reply is sent inside the todo agent separately
        ...
        vector_store.put.assert_any_await(
            ...
            message_id="user-todo-msg-id",
            metadata={"type": "human", "author": "Player", "persona": "Default"},
        )
        vector_store.put.assert_any_await(
            content="Added: buy milk",
            message_id="ai-todo-msg-id",
            metadata={"type": "ai", "author": "todo", "persona": "Default"},
        )
...
async def test_command_storyboard_enabled(mock_session_data):
    ...
    gm_msg = AIMessage(
        content="Last scene description",
        name="Game Master",
        metadata={"message_id": "gm-msg-for-storyboard"},
    )
    ...
    with patch("src.commands.cl.user_session.get", side_effect=user_session_get), patch(
        ...
        "src.commands.storyboard_editor_agent", new_callable=AsyncMock
    ) as mock_storyboard_agent, patch(
        "src.commands.IMAGE_GENERATION_ENABLED", True
    ):  # Ensure enabled
        ...
        mock_cl_message_cls.assert_called_with(
            content="Generating storyboard for the last scene..."
        )
        mock_cl_message_instance.send.assert_awaited_once_with()

        # Check that the agent was called with the correct state and message ID
        mock_storyboard_agent.assert_awaited_once_with(
            state=state, gm_message_id="gm-msg-for-storyboard"
        )
...
async def test_command_storyboard_disabled(mock_session_data):
    ...
    with patch("src.commands.cl.user_session.get", side_effect=user_session_get), patch(
        ...
        "src.commands.IMAGE_GENERATION_ENABLED", False
    ):  # Ensure disabled
...
async def test_command_storyboard_no_gm_message(mock_session_data):
    ...
    state.messages.append(
        AIMessage(content="GM message, no metadata", name="Game Master", metadata={})
    )
...
async def test_command_missing_state():
    ...
    with patch("src.commands.cl.user_session.get", side_effect=user_session_get), patch(
        ...
        mock_cl_message_cls.assert_called_with(
            content="Error: Session state not found."
        )
        mock_cl_message_instance.send.assert_awaited_once_with()
...
async def test_command_help():
    with patch(
        "src.commands.cl.Message", new_callable=MagicMock
    ) as mock_cl_message_cls:
        mock_cl_message_instance = AsyncMock()
        mock_cl_message_instance.send.return_value = None
        mock_cl_message_cls.return_value = mock_cl_message_instance
...
async def test_command_reset(mock_session_data):
    ...
    with patch("src.commands.cl.user_session.get", side_effect=user_session_get), patch(
        "src.commands.cl.user_session.set", new_callable=MagicMock
    ) as mock_user_session_set, patch("src.commands.cl.context", dummy_context), patch(
        "src.commands.cl.Message", new_callable=MagicMock
    ) as mock_cl_message_cls:
...
async def test_command_save(mock_session_data):
    ...
    with patch("src.commands.cl.user_session.get", side_effect=user_session_get), patch(
        "src.commands.cl.context", dummy_context
...
async def test_unknown_slash_command():
    with patch(
        ...
        with patch(
            "src.event_handlers.cl.user_session.get",
...
async def test_command_save_empty_state():
    ...
    with patch("src.commands.cl.user_session.get", return_value=None), patch(
        "src.commands.cl.Message", new_callable=MagicMock
    ) as mock_cl_message_cls:
