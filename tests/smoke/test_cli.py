...
async def test_cli_run_agent(monkeypatch, capsys):
    ...
    with patch("src.cli.get_agent", return_value=dummy_agent), patch("asyncio.get_running_loop", side_effect=RuntimeError):
        main()
        out = capsys.readouterr().out
...
async def test_cli_run_workflow(monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", test_args)
    dummy_supervisor = AsyncMock(return_value=[MagicMock(name="AI", content="Workflow")])
    with patch("src.cli.supervisor", dummy_supervisor), patch("asyncio.get_running_loop", side_effect=RuntimeError):
        main()
        out = capsys.readouterr().out
...
async def test_cli_export_state(tmp_path, monkeypatch, capsys):
    test_args = ["prog", "export-state", "--thread-id", "t1", "--output", str(tmp_path / "state.json")]
    monkeypatch.setattr(sys, "argv", test_args)
