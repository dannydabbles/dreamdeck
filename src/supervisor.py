...
async def supervisor(state: ChatState, **kwargs):
    ...
    while hops < max_hops:
        ...
        if persona_called:
            ...
            # Use explicit "type": "gm_message" metadata to find the GM message
            if state.current_persona.lower().replace(" ", "") in ["storytellergm", "gamemaster"]:
                cl_logger.info("Identified as GM persona, using latest persona message")
                ...
                # Get the last message generated by the persona agent with type "gm_message"
                last_gm_msg = next(
                ...
                if last_gm_msg and last_gm_msg.metadata.get("message_id"):
                    ...
                    cl_logger.warning("No GM message with valid ID found")
            else:
